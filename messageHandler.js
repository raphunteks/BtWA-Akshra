/**
 * ============================================================================
 * 2. MESSAGEHANDLER.JS - LOGIC DISPATCHER & GEMINI AI CLINIC ASSISTANT
 * ============================================================================
...
async function syncLogToGAS(gasApiUrl, payload) {
  ...
  try {
    ...
    }).catch(() => {}); // Tangkap rejection agar tidak memutus eksekusi utama
  } catch (err) {
...
async function askGeminiClinic(apiKey, model, conversationHistory) {
  ...
  const systemInstructionText = 
    ...
    'Pedoman Menjawab:\n' +
    '1. Berikan edukasi kesehatan awal dan panduan konsultasi secara ringkas, jelas, dan bersahabat.\n' +
    '2. Selalu sertakan disclaimer etis bahwa saran ini merupakan triase edukasi awal dan BUKAN pengganti diagnosis medis resmi oleh dokter spesialis.\n' +
    '3. Jika pasien dalam kondisi gawat darurat (sesak napas berat, nyeri dada kiri, pendarahan), anjurkan segera menuju Unit Gawat Darurat (UGD) terdekat.\n' +
    '4. Informasi poli klinik kami: Poli Umum (24 Jam), Poli Gigi, Poli Anak, Poli Penyakit Dalam, Rawat Inap (Ranap) Deluxe & Suite, Laboratorium Cepat, dan Farmasi 24 Jam.';
...
async function handleIncomingMessage(sock, msgUpdate, config) {
  try {
    ...
    // Abaikan pesan dari bot sendiri (fromMe) dan status broadcast
    ...
    // 2. Kirim Sinyal Mengetik (Typing Indicator)
    ...
    if (lowerText === '#menu') {
      commandReply = 
        ...
        '• *#layanan* : Poliklinik & fasilitas rawat inap\n' +
        '• *#lokasi* : Alamat & peta klinik\n' +
        '• *#bantuan* : Panduan konsultasi\n\n' +
      ...
      commandReply = 
        ...
        '2. Poliklinik Rawat Jalan & Dokter Spesialis\n' +
        '3. Layanan Hemodialisa Terpadu\n' +
        '4. Medical Check Up (MCU) Digital Pasien\n' +
        '5. Layanan Antar Resep Farmasi Cepat';
    } else if (lowerText === '#lokasi') {
      commandReply = 
        '*LOKASI & KONTAK RESMI KLINIK*\n\n' +
        '📍 Alamat: Jl. Boulevard Cyber Medical No. 88, Makassar\n' +
        '🌐 Portal Reservasi: Kunjungi portal web klinik kami\n' +
        '📞 Telepon Darurat: (0411) 889-2122';
    } else if (lowerText === '#bantuan') {
      commandReply = 
        '*PANDUAN KONSULTASI BOT MEDIS*\n\n' +
        'Anda dapat berkonsultasi kesehatan langsung secara interaktif. Tanyakan gejala penyakit ringan, penanganan awal demam, dosis vitamin, atau alur registrasi rawat inap.\n\n' +
        '_Peringatan: Seluruh data percakapan dilindungi etika kerahasiaan medis internal._';
    }

    // Jika pesan cocok dengan menu cepat
    if (commandReply) {
    ...
    // 4. Jika bukan Command, Proses via Google Gemini AI (Multi-Turn Chat)
    if (!config.geminiApiKey) {
      const noKeyMsg = 'Mohon maaf, sistem konsultasi AI sedang melakukan sinkronisasi berkala.';
      await sock.sendMessage(remoteJid, { text: noKeyMsg });
      await sock.sendPresenceUpdate('paused', remoteJid);
      return;
    }

    // Ambil riwayat percakapan nomor pasien
    ...
    // Batasi histori 10 percakapan terakhir agar efisien
    ...
    try {
      ...
      // Catat respon AI ke histori multi-turn
      userSession.history.push({
        role: 'model',
        parts: [{ text: aiReply }]
      });
      ...
      aiReply = 'Mohon maaf, sistem konsultasi AI sedang menerima antrean pesan yang cukup tinggi. Silakan ulangi pertanyaan Anda dalam beberapa saat.';
    }

    // Kirim Balasan ke WhatsApp Pasien
    await new Promise((r) => setTimeout(r, 1200));
    await sock.sendMessage(remoteJid, { text: aiReply });
    await sock.sendPresenceUpdate('paused', remoteJid);

    // Simpan Chat Balasan ke Spreadsheet GAS
    syncLogToGAS(config.gasApiUrl, {
      clientId: config.clientId,
      senderNumber: 'BOT_' + config.clientId,
      receiverNumber: senderPhone,
